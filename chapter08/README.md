...
- 딥러닝은 앞에서 언급한 분야 외에도 여러 분야에서 이용되고 있습니다. 이번 절에서는 딥러닝의 가능성과 미래를 느낄 만한 연구를 소개합니다.

### 8.5.1 이미지 스타일(화풍) 변환

- 딥러닝을 활용해 화가차럼 그림을 그리는 다음 연구는 두 이미지를 입력해 새로운 그림을 생성하는 연구입니다.

  - 하나는 콘텐츠 이미지, 하나는 스타일 이미지로서 사용하여 이 둘을 조합해 새로운 그림을 그려줍니다.

  > 예를 들어 고흐의 화풍을 콘텐츠 이미지에 적용하도록 지정하면 이를 기초로 딥러닝이 새로운 그림을 그립니다.

<img src="README.assets/fig 8-23.png" alt="fig 8-23" style="zoom:50%;" />

- 여기서는 이 연구를 자세히 설명하지 않겠지만 큰 틀만 이야기하면 네트워크의 중간 데이터가 콘텐츠 이미지의 중간 데이터와 비슷해지도록 학습합니다.

- 그렇게 하면 입력 이미지를 콘텐츠 이미지 형태를 따라하도록 할 수 있습니다.

- 또한 스타일 이미지의 화풍을 흡수하기 위해 스타일 행렬이라는 개념을 도입해 그 행렬의 오차를 줄임으로서 화풍이 비슷해질 수 있습니다.

### 8.5.2 이미지 생성

- 앞의 이미지 변환이 새 그림을 생성하기 위해 두 장의 이미지를 사용했다면 아무런 이미지 입력 없이 새로운 이미지를 그리는 연구도 진행 중입니다.

> 물론 대중적인 이미지로 학습하긴 하지만 학습이 끝나면 아무런 입력 없이 새 그림을 그립니다.

- 예를 들어 딥러닝으로 침실 이미지를 생성하는 이 연구는 **DCGAN** 기법으로 불립니다.

<img src="README.assets/fig 8-24.png" alt="fig 8-24" style="zoom:50%;" />

- 위 사진들은 진짜처럼 보이는 DCGAN을 사용해 합성한 이미지로 처음부터 새로 생성한 이미지입니다.

- DCGAN은 이미지 생성 과정을 모델링합니다. 그 모델을 대량의 이미지를 사용해 학습하고 학습이 끝나면 그 모델을 이용해 새로운 그림을 생성할 수 있습니다.

- DCGAN도 딥러닝을 사용하며 가장 핵심 기술은 생성자와 식별자로 불리는 두 개의 신경망을 이용한다는 점입니다.

  - 생성자는 진짜와 똑같은 이미지를 생성하고 식별자는 이 이미지가 진짜인지 아닌지 판별합니다.

  - 그렇게 두 신경망이 서로 경쟁하듯 학습하여 더 정교해지는 과정을 **GAN**이라고 부릅니다.

#### Note

- 이전까지 본 기계학습 문제는 **지도 학습** 유형 문제였습니다.

  - 지도 학습이란 손글씨 인식처럼 이미지 데이터와 정답 레이블을 짝지은 데이터셋을 이용합니다.

- 하지만 위와 같은 문제는 지도용 데이터 없이 다량의 데이터로만 스스로 학습하는 **자율 학습** 문제입니다.

  - 자율 학습은 DBF나 DBM과 같이 오래 전부터 연구된 분야지만 최근 침체기를 겪었습니다.

  - 하지만 GAN의 등장으로 새로운 도약을 기대해 볼 수 있을 것입니다.

### 8.5.3 자율 주행

- 사람 대신 컴퓨터가 자동차를 운전하는 **자율 주행** 기술은 다양한 기술을 모아 구현하지만 주변 환경을 올바르게 인식하는 기술이 가장 중요합니다.

- 이는 시시각각 변하는 환경과 종횡무진하는 사람, 차, 사물을 올바르게 인식하기 매우 어렵기 때문입니다.

- 다만, 최근 주위 환경을 인식하는 기술에 딥러닝을 적용한 **SegNet**과 같은 CNN 기반 신경망을 사용한다면 이는 실현가능할지도 모릅니다.

<img src="README.assets/fig 8-25.png" alt="fig 8-25" style="zoom:50%;" />

> SegNet은 이미지를 분할해 픽셀 수준에서 판정하고 있습니다.

### 8.5.4 Deep Q-Network(강화 학습)

- 사람이 시행착오를 겪으며 배워가듯 컴퓨터도 시행착오를 거치며 스스로 학습하는 과정이 바로 **강화 학습**입니다.

- 강화 학습에서는 에이전트가 환경에 맞는 행동을 선택하고 그 행동에 의해 환경이 변하는 것이 기본적인 틀입니다.

  - 환경이 변화하면 에이전트는 어떤 보상을 받습니다. 따라서 강화 학습은 더 나은 보상을 받는 쪽으로 에이전트의 행동 지침을 바로잡아 나갑니다.

<img src="README.assets/fig 8-26.png" alt="fig 8-26" style="zoom:50%;" />

- 이와 같은 강화 학습의 기본 틀을 바탕으로 개발하되 주의할 점은 보상은 정해진 것이 아니라 예상 보상이라는 점입니다.

  - 예를 들어 슈퍼 마리오 브라더스 게임에서 마리오가 오른쪽으로 이동할 때 보상은 항상 명확한 것이 아닙니다.

  - 즉, 어떤 상황에서 이동한 것이냐에 따라 보상이 천차만별이므로 이런 불명확한 보상은 명확한 지표에서 역산해서 예상 보상을 정해야 합니다.

  - 만약 지도 학습이라면 행동에 따라 지도하여 올바른 평가를 받게 할 수 있습니다.

- 딥러닝을 사용한 강화 학습 중 **Deep Q-Network**, 이른바 DQN이 있습니다.

  - 이는 Q 학습이라는 강화 학습 알고리즘을 기초로 하는데 Q 학습은 최적 행동 가치 함수로 최적인 행동을 정합니다.

  - 이 함수를 딥러닝(CNN)으로 비슷하게 흉내 내어 사용하는 것이 바로 DQN입니다.

> DQN을 활용한 연구는 비디오 게임을 자율 학습 시켜 사람을 뛰어넘는 수준의 조작을 실현한 사례가 보고되고 있습니다.

- DQN은 다음과 같이 CNN에서 영상 프레임을 입력해 최종적으로 제어하는 움직임에 대한 각 동작의 가치를 출력합니다.

<img src="README.assets/fig 8-27.png" alt="fig 8-27" style="zoom:50%;" />

- 그간 비디오 게임 학습이 게임의 상태를 미리 추출하는 것이 보통이었다면 DQN은 비디오 영샹만을 보고 결과를 추출합니다.

- 이것이 바로 DQN의 주목할 점으로 DQN의 응용 가능성을 현격히 높였다고 할 수 있습니다.

  - 예를 들어 게임마다 설정을 바꾸지 않고 영상을 보여주기만 하면 DQN이 많은 게임을 학습할 수 있게 해줍니다.

#### Note

- AI인 알파고가 이세돌을 꺾었던 것도 딥러닝과 강화 학습 덕분입니다.

  - 알파고는 3000만 개의 기보를 보며 학습한 후 스스로 자기와 맞붙는 대결을 반복하여 수련했습니다.

> 알파고와 DQN은 모두 구글이 인수한 딥마인드가 진행한 연구입니다.

## 8.6 정리

- 수많은 문제에서 신경망을 더 깊게 하여 성능을 개선할 수 있습니다.

- 이미지 인식 기술 대회인 ILSVRC에서 최근 딥러닝 기반 기법이 상위권을 독점 중이며 그 깊이도 더 깊어지는 추세입니다.

- 유명한 신경망으로는 VGG, GoogLeNet, ResNet이 있습니다

- GPU와 분산 학습, 비트 정밀도 감소 등으로 딥러닝을 고속화할 수 있습니다.

- 딥러닝은 사물 인식 뿐만 아니라 사물 검출과 분할에도 사용할 수 있습니다.

- 딥러닝의 응용 분야로 사진의 캡션 생성, 이미지 생성, 강화 학습 등이 있으며 최근엔 자율 주행에도 활용됩니다.
